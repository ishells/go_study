...
/*
	...
	**注意：**一个通道值是可以被垃圾回收掉的。通道通常由发送方执行关闭操作，并且只有在接收方明确等待通道关闭的信号时才需要执行关闭操作。它和关闭文件不一样，通常在结束操作之后关闭文件是必须要做的，但关闭通道不是必须的。
	...
	无缓冲的通道
	容量为零的通道就是无缓冲的通道，无缓冲的通道又称为阻塞的通道。
    ...
    另外，对一个只接收通道执行close也是不允许的，因为默认通道的关闭操作应该由发送方来完成。
	...
	select多路复用
		...
		上面这个例子中：
		1、<-ch1: 这个case表示从ch1通道接收数据。如果ch1通道中有数据可接收，则执行对应的操作。
			如果ch1通道为空，则这个case会被阻塞，直到ch1通道中有数据可接收。
		2、data := <-ch2: 这个case表示从ch2通道接收数据，并将接收到的数据赋值给data变量。
			如果ch2通道中有数据可接收，则执行对应的操作。如果ch2通道为空，则这个case会被阻塞，直到ch2通道中有数据可接收。
		3、ch3 <- 10: 这个case表示向ch3通道发送数据10。如果ch3通道可以接收数据，则将10发送到ch3通道中。
			如果ch3通道已满，则这个case会被阻塞，直到ch3通道有空间可用。
		4、default: 这是select语句中的默认情况。如果所有的case都没有准备好（通道为空或通道已满），则会执行default中的操作。
		在这个示例中，如果没有任何一个通道准备好，则执行默认操作。
